...
class AES
  ...
  def decrypt_cbc(str, key)
    prepare_for_decryption(str, key)
    do_decrypt_cbc
  end
...
private
  ...
  def prepare_for_decryption(str, key)
    @key = Utils.hex2str(key)
    arr = Utils.hex2str(str)
    @iv = arr[0, block_size]
    @data = arr[block_size..-1]
    # decryptor
  ...
  def do_decrypt_ctr
    @result = ''
    @data.each_slice(block_size) do |slice|
      slice = align_block(slice)
      dec = decrypt_block(@iv)
      @result += dec ^ slice
    end
    @result
  end
  ...
  def align_block(block)
    if (delta = block_size - block.size) > 0
